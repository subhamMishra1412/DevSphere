  ...
  const onSubmit = async (data) => {
    ...
    try {
      if (auth?.login) {
        ...
        // Fallback simulation if auth context is bypassed during UI testing
        await new Promise((resolve) => setTimeout(resolve, 1200));
      }

      toast.success("Authentication successful", {
        description: "Welcome back to your DevSphere workspace.",
      });

      navigate("/dashboard");
    } catch (error) {
      toast.error("Authentication failed", {
        description: error?.message || "Invalid work email or password credentials.",
      });
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="w-full space-y-6 select-none">
      ...
      <div className="space-y-1.5 text-center sm:text-left">
        ...
        <p className="text-xs sm:text-sm text-zinc-400">
          Enter your professional credentials to access active sprints.
        </p>
      </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              <label className="text-xs font-bold uppercase tracking-wider text-zinc-300" htmlFor="password">
                Password
              </label>
              <button
                type="button"
                onClick={() => toast.info("Password reset link dispatched to IT admin.")}
                className="text-[11px] font-medium text-indigo-400 hover:text-indigo-300 transition hover:underline"
              >
                Forgot password?
              </button>
            </div>
